<?php
/*
	##! 缺陷： 如果选择其他的库，或者其他的链接。  （可以每个类生成一个校验码来判断）
*/
...
class isMysql {
	...
	/*
		构造函数生成$this->conn，并选择操作的库		
	*/
	final protected function __construct( $host, $user, $pass, $db ) {
	...
	/*	初始化获取该对象
		...
		$db  选择的库
	*/
	public static function getIni( $host, $user, $pass,$db ) {
		if( self::$ins === null ) {
			self::$ins = new self($host, $user, $pass,$db);
		}
		return self::$ins;
	}
	...
	/*
		...
		$where 条件
	*/
	public function select( $tab, $where = array(), $filed=array(), $order=' id asc ', $limit = '' ){
		$filedStr = '*';
		$whereStr = '';
		$orderStr = $order;
		$limitStr = $limit;
		...
		$sqlStr = 'select '. $filedStr .' from '.$tab. $whereStr. ' order by '.$orderStr. $limitStr;
		/*if( empty($where) ){
			$collect = mysql_query($sqlStr, $this->conn);
		}else{
			// SELECT * FROM `message` where id = 1;
			$collect = mysql_query($sqlStr, $this->conn);
		}*/
		return $this->format( mysql_query( $sqlStr, $this->conn ) );
	}
